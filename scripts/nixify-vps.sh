...
#ssh $SSH_HOST nix-shell -p git --command '"cd machine-configuration/this-vps; git add hardware-configuration.nix networking.nix -f"'
#
#ssh $SSH_HOST mkdir /root/.ssh
#ssh $SSH_HOST "cat - > /root/.ssh/known_hosts" << EOF
