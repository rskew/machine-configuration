...
if [ -n "$(echo $DISPLAY_CONNECTED | grep VGA-1)" ]; then
  ...
  DISPLAY=:0 xrandr --output eDP-1 --mode 3840x2160 --pos 0x2100 --output DP-2-3 --scale 2 --primary --mode 1680x1050 --pos 0x0

elif [ -n "$(echo $DISPLAY_CONNECTED | grep DP-3-3)" ]; then
  DISPLAY=:0 xrandr --output eDP-1 --mode 3840x2160 --pos 0x2100 --output DP-3-3 --scale 2 --primary --mode 1680x1050 --pos 0x0
